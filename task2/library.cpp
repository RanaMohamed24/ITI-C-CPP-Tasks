...
int getInput()
{
    ...
    if (ch == 13) return KeyEnter;
    if (ch == 27) return KeyEsc;
    if (ch == 8  || ch == 127) return KeyBackspace;
...
static Key readKey()
{
    ...
    if (c == 27)
    {
        ...
        {
            unsigned char b1;
            enableRaw(); read(STDIN_FILENO, &b1, 1); disableRaw();
            if (b1 == '[')
            {
                unsigned char b2;
                enableRaw(); read(STDIN_FILENO, &b2, 1); disableRaw();
                switch (b2)
                {
                    case 'A': return KeyUp;
